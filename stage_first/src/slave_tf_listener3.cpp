...
int main(int argc, char **argv) {
  ...
  while (node.ok()) {
    ...
    try {
      listener.lookupTransform("/ares3", "/ares1", ros::Time(0), transformSM);
    ...
    try {
      listener.lookupTransform("/ares1", "/ares3", ros::Time(0), transformMS);
    } catch (tf::TransformException &ex) {
      ROS_ERROR("%s", ex.what());
      ros::Duration(1.0).sleep();
      continue;
    ...
    // Priting the x, y, and angle "from the robot's perspective" of each robot
    // to the other
    ROS_INFO("Master pose w.r.t Slave [x, y]: [%f, %f]",
             transformSM.getOrigin().x(), transformSM.getOrigin().y());
    ROS_INFO("Orientation: [%f]",
             atan2(transformSM.getOrigin().y(), transformSM.getOrigin().x()));
    ROS_INFO("---------------------------------------------");
    ROS_INFO("Slave pose w.r.t Master [x, y]: [%f, %f]",
             transformMS.getOrigin().x(), transformMS.getOrigin().y());
    ROS_INFO("Orientation: [%f]",
             atan2(transformMS.getOrigin().y(), transformMS.getOrigin().x()));
    // Proportional controller to follow the Master (Go to master behaviour)
    geometry_msgs::Twist vel_msg;
    vel_msg.angular.z =
        4.0 * atan2(transformSM.getOrigin().y(), transformSM.getOrigin().x());
...
void laserCallBack(const sensor_msgs::LaserScan::ConstPtr &laser_msg) {
  ...
  for (int i = 0; i < 27;
       i++) // I need not loop to copy, I not familiar with std::vectors
  {
...
void move(double speed, double distance, bool isForward) {
  geometry_msgs::Twist vel_msg;
  // set a random linear velocity in the x-axis and condition to determine
